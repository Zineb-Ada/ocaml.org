...
let blog req =
  let filter =
    Dream.query req "f" |> Option.value ~default:"all"
  in
  let filtred_items_by_tags = List.filter (fun item -> List.mem filter item.Data.Planet.tags) Data.Planet.all
  in
  let page, number_of_pages, current_items =
    paginate ~req ~n:10
      (List.filter (fun (x : Data.Planet.t) -> not x.featured) filtred_items_by_tags)
  in
